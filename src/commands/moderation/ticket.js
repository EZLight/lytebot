  ...
  async execute(interaction) {
    ...
    switch (sub) {
      case "send":
        if (!data)
          return await interaction.reply({
            content: `⚠️ You have to do /ticket setup before you can send a ticket message...`,
            ephemeral: true,
          });
      ...
      case "remove":
        if (!data)
          return await interaction.reply({
            content: `⚠️ Looks like you don't already have a ticket system set`,
            ephemeral: true,
          });
        else {
          await ticket.deleteOne({ Guild: interaction.guild.id });
          await interaction.reply({
            content: `🎛️ I have deleted your ticket category.`,
            ephemeral: true,
          });
        }
